...
&nbsp; id, email, password\_hash, role (author|student)
...
&nbsp; id, author\_id, name, description, status (draft|published)

&nbsp; scenario\_type: str  # lecture\_free | case\_trainer
...
&nbsp; id, course\_id, filename, s3\_key, status (processing|ready|error)



ContentChunk:

&nbsp; id, course\_id, content, embedding (vector), metadata: JSON



Diagnostic:

&nbsp; id, section\_id, type, question, options: JSON, correct\_answer
...
&nbsp; completed\_sections: JSON  # список ID

&nbsp; diagnostic\_results: JSON  # {diagnostic\_id: score}
...
&nbsp;       {"name": "Раздел 1", "key\_points": \["тезис 1", "тезис 2"]},
...
Ты — AI-компаньон курса "{course\_name}".
...
\- Стиль общения: {companion\_style}
